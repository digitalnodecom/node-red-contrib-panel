...
// Reserved field names that cannot be used by users
    ...
    fields: Joi.array().items(
        Joi.object({
            name: Joi.string()
                ...
                .required()
                .messages({
                    'any.invalid': `Field name cannot be one of the reserved names: ${RESERVED_FIELD_NAMES.join(', ')}`
                }),
            type: Joi.string()
...
const validateCollection = (req, res, next) => {
    ...
    if (fields && Array.isArray(fields)) {
        ...
        if (duplicates.length > 0) {
            return res.status(400).json({
                error: 'Validation error',
                details: [`Duplicate field names are not allowed: ${[...new Set(duplicates)].join(', ')}`]
            });
        }
    }
