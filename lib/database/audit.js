...
/**
 * Log an audit event to the main database
 */
const logAuditEvent = async (mainDb, options) => {
    const {
        action_type,
        entity_type,
        entity_id = null,
 ...
 */
const auditDatabase = {
    created: (mainDb, dbName, displayName, req) => logAuditEvent(mainDb, {
    ...
    updated: (mainDb, dbName, displayName, changes, req) => logAuditEvent(mainDb, {
    ...
    deleted: (mainDb, dbName, displayName, req) => logAuditEvent(mainDb, {
    ...
    setDefault: (mainDb, dbName, displayName, req) => logAuditEvent(mainDb, {
    ...
    journalModeChanged: (mainDb, dbName, oldMode, newMode, req) => logAuditEvent(mainDb, {
...
const auditCollection = {
    created: (mainDb, collectionName, databaseContext, fieldCount, req) => logAuditEvent(mainDb, {
    ...
    updated: (mainDb, collectionName, databaseContext, changes, req) => logAuditEvent(mainDb, {
    ...
    deleted: (mainDb, collectionName, databaseContext, req) => logAuditEvent(mainDb, {
    ...
    truncated: (mainDb, collectionName, databaseContext, recordCount, req) => logAuditEvent(mainDb, {
 ...
 */
const getRecentAuditLog = async (mainDb, options = {}) => {
    const {
        limit = 20,
        offset = 0,
        action_type = null,
    ...
    try {
        ...
        const entries = await mainDb.all(query, params);
 ...
 */
const getAuditStats = async (mainDb) => {
    try {
        ...
        const recentActions = await mainDb.all(`
 ...
 */
const cleanupAuditLog = async (mainDb, retentionDays = 90) => {
    try {
