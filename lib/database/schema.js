const initSchema = async (db) => {
    try {
        ...
        // Database registry table for multi-database support
        await db.exec(`
        ...
        // Audit log table for tracking all system changes with user information
        await db.exec(`
        ...
        // Migration: Add journal_mode column to existing _databases table if it doesn't exist
        try {
            const databasesColumns = await db.all("PRAGMA table_info(_databases)");
            const hasJournalMode = databasesColumns.some(col => col.name === 'journal_mode');

            if (!hasJournalMode) {
                console.log('Migrating database: adding journal_mode column to _databases table');
                await db.exec('ALTER TABLE _databases ADD COLUMN journal_mode TEXT DEFAULT \'WAL\'');
            }
        } catch (migrationError) {
            console.warn('Migration warning (may be safe to ignore):', migrationError.message);
        }

        // Migration: Rename is_master column to is_main in _databases table
        try {
            const databasesColumns = await db.all("PRAGMA table_info(_databases)");
            const hasIsMaster = databasesColumns.some(col => col.name === 'is_master');
            const hasIsMain = databasesColumns.some(col => col.name === 'is_main');

            if (hasIsMaster && !hasIsMain) {
                console.log('Migrating database: renaming is_master to is_main in _databases table');
                ...
                await db.exec(`
                    ...
                    -- Copy data (mapping is_master to is_main)
                    INSERT INTO _databases_new (id, name, display_name, file_path, description, is_active, is_main, is_default, journal_mode, settings, created_at, updated_at)
                    SELECT id, name, display_name, file_path, description, is_active, is_master, is_default, journal_mode, settings, created_at, updated_at
                    FROM _databases;

                    -- Drop old table
                    DROP TABLE _databases;

                    -- Rename new table
                    ALTER TABLE _databases_new RENAME TO _databases;

                    COMMIT;
                `);

                console.log('Migration complete: is_master renamed to is_main');
            }
        } catch (migrationError) {
            console.warn('Migration warning for is_master->is_main (may be safe to ignore):', migrationError.message);
        }
        ...
        // Create trigger to update databases timestamp
        await db.exec(`
...
// Separate schema initialization for user databases (excludes master-only tables)
const initUserSchema = async (db) => {
    try {
        console.log('Panel: Initializing user database schema (without system tables)');
        ...
        try {
            ...
            if (!hasIndexable) {
                console.log('Migrating user database: adding indexable column to _fields table');
                await db.exec('ALTER TABLE _fields ADD COLUMN indexable INTEGER DEFAULT 0');
            }
        } catch (migrationError) {
            console.warn('Migration warning (may be safe to ignore):', migrationError.message);
        ...
        try {
            ...
            if (!hasEventsEnabled) {
                console.log('Migrating user database: adding events_enabled column to _collections table');
                await db.exec('ALTER TABLE _collections ADD COLUMN events_enabled INTEGER DEFAULT 0');
            }
        } catch (migrationError) {
            console.warn('Migration warning (may be safe to ignore):', migrationError.message);
        ...
        console.log('Panel: User database schema initialized successfully');
...
const getCollections = async (db) => {
    ...
    for (const collection of collections) {
        try {
            // Get field count from _fields table
...
const updateCollection = async (db, id, updates) => {
    ...
    try {
        ...
        const oldTableName = currentCollection.name;
        const newTableName = name || currentCollection.name;
        
        // PHASE 1: Validate and prepare SQLite operations (SQLite-first approach)
        // Get current table structure
        const currentTableInfo = await db.all(`PRAGMA table_info(${oldTableName})`);
        const currentColumns = currentTableInfo.map(col => col.name);
        
        // Detect field changes
        const changes = analyzeFieldChanges(currentFields, newFields, currentColumns);
        
        // PHASE 2: Execute SQLite table modifications FIRST
        await executeTableModifications(db, oldTableName, newTableName, changes, newFields);
        
        // PHASE 3: Only update metadata if SQLite operations succeeded
        await updateCollectionMetadata(db, id, name, newFields, events_enabled, currentCollection);
        
        // PHASE 4: Handle indexes and triggers
        await updateIndexesAndTriggers(db, newTableName, newFields, events_enabled);
        
        await db.run('COMMIT');
        console.log(`Collection ${newTableName} updated successfully`);
...
// Helper function to analyze what changes need to be made to the table
const analyzeFieldChanges = (currentFields, newFields, currentColumns) => {
    const changes = {
        additions: [],
        renames: [],
        removals: [],
    ...
    for (const field of newFields) {
        if (!currentFieldNames.includes(field.name) && !currentColumns.includes(field.name)) {
    ...
    for (const field of currentFields) {
        if (!newFieldNames.includes(field.name)) {
            changes.removals.push(field);
        }
    }
    
    // Detect modifications to existing fields
    for (const newField of newFields) {
        const currentField = currentFields.find(f => f.name === newField.name);
        if (currentField && currentColumns.includes(newField.name)) {
            // Check if any properties have changed that require table modification
            const hasChanges = 
                currentField.type !== newField.type ||
                Boolean(currentField.required) !== Boolean(newField.required) ||
                Boolean(currentField.unique) !== Boolean(newField.unique) ||
                currentField.default !== newField.default;
            ...
            if (hasChanges) {
                changes.modifications.push({
                    ...
                    newField: newField,
                    changes: {
                        type: currentField.type !== newField.type,
                        required: Boolean(currentField.required) !== Boolean(newField.required),
                        unique: Boolean(currentField.unique) !== Boolean(newField.unique),
                        default: currentField.default !== newField.default
                    }
                });
            }
        }
...
// Helper function to execute table modifications
const executeTableModifications = async (db, oldTableName, newTableName, changes, newFields) => {
    let needsTableRecreation = false;
    
    // Check if we need table recreation (for renames, constraint changes, etc.)
    if (oldTableName !== newTableName || changes.removals.length > 0 || changes.renames.length > 0 || changes.modifications.length > 0) {
        needsTableRecreation = true;
    }
    
    // Log what triggered table recreation
    if (needsTableRecreation) {
        console.log(`Table recreation required for ${oldTableName}:`);
        if (oldTableName !== newTableName) console.log(`- Name change: ${oldTableName} → ${newTableName}`);
        if (changes.removals.length > 0) console.log(`- Removed fields: ${changes.removals.map(f => f.name).join(', ')}`);
        if (changes.modifications.length > 0) {
            console.log(`- Modified fields: ${changes.modifications.map(m => `${m.name} (${Object.keys(m.changes).filter(k => m.changes[k]).join(', ')})`).join(', ')}`);
        }
        await recreateTable(db, oldTableName, newTableName, newFields);
    } else {
        // Simple column additions
        for (const field of changes.additions) {
            const sqlType = getSQLType(field.type);
            let alterSQL = `ALTER TABLE ${oldTableName} ADD COLUMN ${field.name} ${sqlType}`;
            ...
            console.log(`Added column ${field.name} to table ${oldTableName}`);
        }
    }
};

// Helper function to recreate table with new schema (for complex changes)
const recreateTable = async (db, oldTableName, newTableName, newFields) => {
    const tempTableName = `${newTableName}_temp_${Date.now()}`;
    ...
    // Copy data from old table to new table (matching columns only)
    const oldTableInfo = await db.all(`PRAGMA table_info(${oldTableName})`);
    const newTableInfo = await db.all(`PRAGMA table_info(${tempTableName})`);
    
    const oldColumns = oldTableInfo.map(col => col.name);
    ...
    if (commonColumns.length > 0) {
        ...
        await db.exec(`INSERT INTO ${tempTableName} (${columnsStr}) SELECT ${columnsStr} FROM ${oldTableName}`);
        console.log(`Copied data from ${oldTableName} to ${tempTableName}`);
    }
    
    // Drop old table and rename temp table
    await db.exec(`DROP TABLE ${oldTableName}`);
    await db.exec(`ALTER TABLE ${tempTableName} RENAME TO ${newTableName}`);
    console.log(`Renamed ${tempTableName} to ${newTableName}`);
    
    // Recreate update trigger for the new table
    await db.exec(`
        CREATE TRIGGER IF NOT EXISTS update_${newTableName}_timestamp 
        AFTER UPDATE ON ${newTableName}
        BEGIN
            UPDATE ${newTableName} SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
        END;
    `);
};

// Helper function to update collection metadata
const updateCollectionMetadata = async (db, id, name, newFields, events_enabled, currentCollection) => {
    ...
    await db.run(
        ...
        [
            ...
            events_enabled !== undefined ? (events_enabled ? 1 : 0) : currentCollection.events_enabled, 
            id
        ]
    );
...
// Helper function to update indexes and triggers
const updateIndexesAndTriggers = async (db, tableName, newFields, events_enabled) => {
