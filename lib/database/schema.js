const initSchema = async (db) => {
    try {
        ...
        // API keys table for external access authentication
        await db.exec(`
            CREATE TABLE IF NOT EXISTS _api_keys (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                name TEXT NOT NULL,
                key_hash TEXT UNIQUE NOT NULL,
                permissions TEXT DEFAULT '{"read": true, "write": false, "collections": ["*"]}',
                database_permissions TEXT DEFAULT '{}',
        ...
        // Create trigger to update API keys timestamp
        await db.exec(`
            CREATE TRIGGER IF NOT EXISTS update_api_keys_timestamp 
            AFTER UPDATE ON _api_keys
            BEGIN
                UPDATE _api_keys SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
            END
        `);
