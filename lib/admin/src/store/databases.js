...
export const useDatabasesStore = defineStore('databases', {
  ...
  getters: {
    ...
    getMainDatabase: (state) => {
      return state.databases.find(db => db.is_main === 1)
    },
  ...
  actions: {
    async fetchDatabases() {
      ...
      try {
        ...
        if (this.currentDatabase === 'main' && data.length > 0) {
    ...
    async deleteDatabase(dbId) {
      ...
      try {
        ...
        // Switch to main if current database was deleted
    ...
    initializeCurrentDatabase() {
      ...
      if (stored && this.databases.find(db => db.name === stored)) {
        ...
        // Use default database or main
        const defaultDb = this.databases.find(db => db.is_default === 1)
        this.currentDatabase = defaultDb ? defaultDb.name : 'main'
      }
    }
  }
})