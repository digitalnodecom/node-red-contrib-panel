module.exports = function(RED) {
    ...
    function PanelInsertNode(config) {
        ...
        const processInsert = async (msg, send, done) => {
            ...
            // Use config.data if msg.payload is not a valid object for insertion
            if ((!data || typeof data !== 'object' || Array.isArray(data)) && config.data) {
                try {
    ...
    function PanelUpdateNode(config) {
        ...
        const processUpdate = async (msg, send, done) => {
            ...
            // Use config.data if msg.payload is not a valid object for update
            if ((!payload || typeof payload !== 'object' || Array.isArray(payload)) && config.data) {
                try {
    ...
    function PanelDeleteNode(config) {
        ...
        const processDelete = async (msg, send, done) => {
            ...
            // Use msg.payload or fallback to config data
            let id;
            // Check if msg.payload is a valid ID (number or object with id property)
            if (msg.payload !== undefined && msg.payload !== null &&
                (typeof msg.payload === 'number' ||
                 (typeof msg.payload === 'object' && !Array.isArray(msg.payload) && msg.payload.id))) {
    ...
    function PanelUpsertNode(config) {
        ...
        const processUpsert = async (msg, send, done) => {
            ...
            // Use msg.payload or fallback to config data
            let data = msg.payload;
            // Use config.data if msg.payload is not a valid object for upsert
            if ((!data || typeof data !== 'object' || Array.isArray(data)) && config.data) {
                try {
                    data = JSON.parse(config.data);
                } catch (e) {
