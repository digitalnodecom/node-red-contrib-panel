module.exports = function(RED) {
    ...
    const corsOptions = {
        origin: function (origin, callback) {
            ...
            // For now, allow all origins but with credentials
            callback(null, true);
        },
        credentials: true, // Allow cookies to be sent with requests
        optionsSuccessStatus: 200
    };
    app.use('/panel/api', cors(corsOptions));
    ...
    // Authentication middleware for admin UI
    const authenticateAdminUI = (req, res, next) => {
        ...
        // Check if user is authenticated via Node-RED session
        // Node-RED sets req.user when a user is logged in
        if (req.user) {
            return next();
        }
        
        // Check if this is an AJAX request (don't redirect API calls)
        if (req.xhr || req.headers.accept && req.headers.accept.indexOf('json') > -1) {
            return res.status(401).json({
                error: 'Authentication required',
                details: 'Please log in to Node-RED to access the panel'
            });
        }
        
        // Redirect to Node-RED login page
        // Store the original URL to redirect back after login
        const returnUrl = req.originalUrl || '/panel';
        res.redirect(`/?return=${encodeURIComponent(returnUrl)}`);
    };

    // Serve Vue admin UI with authentication check
    const adminPath = path.join(__dirname, 'admin', 'dist');
    
    // Apply authentication middleware to all /panel routes except API
    app.use('/panel', authenticateAdminUI, express.static(adminPath));
    
    // Catch-all route for Vue SPA with authentication check
    app.get('/panel/*', authenticateAdminUI, (req, res) => {
