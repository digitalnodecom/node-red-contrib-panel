module.exports = function(RED) {
    ...
    // Internal token endpoint for Node-RED editor authentication (no auth required - only accessible to Node-RED editor)
    app.get('/panel/internal-token', (req, res) => {
        // Always return null token since we're removing auth requirements from node endpoints
        res.json({ token: null });
    });
    
    // Node-RED specific API endpoint to fetch databases for dropdowns (no auth required - internal Node-RED use)
    app.get('/panel/node-databases', async (req, res) => {
        try {
    ...
    // Node-RED specific API endpoint to fetch collections for dropdowns (no auth required - internal Node-RED use)
    ...
    // Node-RED specific API endpoint to fetch event-enabled collections (no auth required - internal Node-RED use)
    ...
    // Node-RED specific API endpoint to fetch collection field details (no auth required - internal Node-RED use)
    app.get('/panel/node-collection-fields', async (req, res) => {
        try {
            // Ensure database manager is initialized
            await initializeDatabaseManager();
